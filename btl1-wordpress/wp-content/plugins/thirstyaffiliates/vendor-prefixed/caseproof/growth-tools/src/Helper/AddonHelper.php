...
namespace ThirstyAffiliates\Caseproof\GrowthTools\Helper;
    ...
    public static function activateAddon(string $addon, string $addonType = 'plugin')
    {
        ...
        if (check_ajax_referer('caseproof_growth_tools_install_addon', 'nonce')) {
            ...
            if (! current_user_can('activate_plugin')) {
                wp_send_json_error(
                    esc_html__('Could not activate addon. Please check user permissions.', 'thirstyaffiliates')
                );
            }
            ...
            if (isset($addon)) {
                ...
                if (! is_wp_error($activate)) {
                    wp_send_json_success(esc_html__('Addon activated.', 'thirstyaffiliates'));
                }
            }

            wp_send_json_error(
                esc_html__('Could not activate addon. Please activate from the Plugins page.', 'thirstyaffiliates')
            );
        }

        wp_send_json_error(
            esc_html__('Could not activate addon. Please refresh page and try again.', 'thirstyaffiliates')
        );
    }
