...
### 1. Phần Giới thiệu (Về OrderFlow & Số liệu)
* **`container`** *(Vị trí: dòng 120, 166, 208...)*: Tạo một khung chứa bao bọc toàn bộ nội dung bên trong, tự động căn giữa trang và thiết lập max-width an toàn theo từng kích thước màn hình, giúp nội dung không bị tràn sát lề.
* **`row`** *(Vị trí: dòng 121, 133, 170...)*: Khởi tạo một hàng (sử dụng flexbox) để chứa các cột (`col-*`), xử lý triệt để các rắc rối về margin âm mặc định của Bootstrap.
* **`align-items-center`** *(Vị trí: dòng 121)*: Sử dụng flexbox để căn giữa theo trục dọc (vertical alignment) cho các phần tử con bên trong.
* **`gy-5`** *(Vị trí: dòng 121, 252...)*: Tạo khoảng cách (gutter) theo trục Y (chiều dọc) giữa các cột/hàng với kích thước mức độ 5 (rất lớn).
* **`col-lg-6`, `col-lg-5`, `col-lg-7`** *(Vị trí: dòng 122, 253...)*: Định nghĩa độ rộng của cột trên màn hình lớn (Large - từ 992px trở lên). Ví dụ `col-lg-6` sẽ chiếm 6/12 phần (tương đương 50% chiều rộng).
...
### 3. Phần Liên hệ
* **`btn`, `btn-lg`** *(Vị trí: dòng 301)*: Định nghĩa cấu trúc chuẩn của một nút bấm (nền, viền, khoảng cách, font) và phiên bản phóng to (`btn-lg`).
